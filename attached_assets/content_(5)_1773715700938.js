...
async function getDescMax() {
  return new Promise(function (resolve) {
    chrome.storage.sync.get("autimik_settings", function (result) {
      var settings = result.autimik_settings || {};
      resolve(settings.descMax || 4096);
    });
  });
}
...
async function fillForm(vehicle) {
  ...
  // Apply descMax to the Facebook adapter's description field dynamically
  var adapter = await findAdapter(window.location.href);
  var summary;

  if (adapter) {
    // Override description maxLength with user setting
    if (adapter.fields && adapter.fields.description) {
      adapter = JSON.parse(JSON.stringify(adapter)); // shallow clone
      adapter.fields.description.maxLength = descMax;
    }
