...
// ─── In-memory CRM mapping store (replace with DB in production) ──────────────
...
function scoreValueMatch(samples: string[], field: FieldDef): number {
  ...
  return Math.round((hits / samples.length) * 40); // up to 40 bonus points
}
...
export function registerCsvImportRoutes(app: Express) {
  ...
  app.post("/api/csv-import/analyze", upload.single("file"), async (req, res) => {
    try {
      ...
      const hasSavedMappings = !!savedMappings;
  ...
  // Step 4: Import with confirmed mappings
  app.post("/api/csv-import/import", upload.single("file"), async (req, res) => {
    try {
      ...
      // Reverse mapping: csvHeader → vehicleFieldKey
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          // Require at minimum: year + make + model OR stockNumber
          ...
          // Ensure required title field
          if (!vehicleData.title) vehicleData.title = vehicleData.stockNumber || "Untitled Vehicle";
          if (!vehicleData.vin) vehicleData.vin = vehicleData.stockNumber || `IMPORT-${Date.now()}-${i}`;

          // Check for existing vehicle by stockNumber
          const stockNumber = vehicleData.stockNumber;
          let existing: any = null;

          if (stockNumber) {
            const allVehicles = await storage.searchVehicles(stockNumber);
            existing = allVehicles.find((v: any) => (v as any).stockNumber === stockNumber);
          }
