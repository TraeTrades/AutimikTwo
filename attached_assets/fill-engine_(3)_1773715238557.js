var FillEngine = (function () {
  ...
  async function fillCheckboxField(selectors, value, options) {
    options = options || {};
    var label = options.label || "checkbox";
    var shouldCheck = value !== false && value !== "false" && value !== 0;
    // Default true if no value provided
    if (value === undefined || value === null || value === "") shouldCheck = true;
    try {
      // FB's clean title checkbox is inside a label — find it by nearby text first
      var el = null;
      var labelText = "This vehicle has a clean title";
      var spans = document.querySelectorAll("span, div, label");
      for (var i = 0; i < spans.length; i++) {
        if (spans[i].textContent.trim() === labelText) {
          var container = spans[i].closest("label") || spans[i].parentElement;
          if (container) {
            el = container.querySelector("input[type=\"checkbox\"]") || container;
          }
          break;
        }
      }
      // Fallback to CSS selectors
      if (!el) el = await waitFor(selectors, options.timeout || 4000);
      ...
      if (el.tagName === "INPUT" && el.type === "checkbox") {
        ...
        // The element itself may be a clickable container acting as checkbox
      ...
      return { field: label, status: "filled", value: shouldCheck };
    } catch (e) {
      log("Could not set checkbox", label, ":", e.message);
      return { field: label, status: "failed", error: e.message };
    }
  }
  ...
  async function fillField(fieldConfig, value, options) {
    ...
    switch (fieldConfig.type) {
      case "checkbox":
        return await fillCheckboxField(selectors, value, fieldOptions);
      case "typeahead":
  ...
  return {
    ...
    fillCheckboxField: fillCheckboxField,
