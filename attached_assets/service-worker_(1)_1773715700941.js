...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  ...
  switch (type) {
    ...
    case "SAVE_SETTINGS": {
      chrome.storage.sync.set({ autimik_settings: payload.settings }, function () {
        sendResponse({ success: true });
      });
      return true;
    }
    ...
    case "GET_SETTINGS": {
      chrome.storage.sync.get("autimik_settings", function (result) {
        sendResponse({ success: true, settings: result.autimik_settings || {} });
      });
      return true;
    }

    case "SAVE_SIGNATURE": {
      chrome.storage.sync.set({ autimik_signature: payload.signature }, function () {
        sendResponse({ success: true });
      });
      return true;
    }
    ...
    case "GET_SIGNATURE": {
      chrome.storage.sync.get("autimik_signature", function (result) {
        sendResponse({ success: true, signature: result.autimik_signature || "" });
      });
      return true;
    }
